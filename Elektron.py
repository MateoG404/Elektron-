...
def ventanaGraficas(fig,numero):
    ...
    modificarButton=Button(optionsFrame,text = "Modificar",font=("Bahnschrift Light",10),bg="#FFFFFF",fg="#000000",width=8,height=1, command = lambda: modificarValores(numero))
...
def construirGraficas():
    ...
    if Show[1]==True:
        ...
        plt.contourf(X, Y, V_e[0], 30, cmap=plt.cm.Reds, linestyles="dashed")
        plt.contour(X, Y, -V_e[1], 30, colors='k', linestyles="dashed")#Porque tenia 20
        plt.contourf(X, Y, -V_e[1], 30, cmap=plt.cm.Blues,)#Porque tenia 20
